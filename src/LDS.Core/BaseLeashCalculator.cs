...
internal class BaseLeashCalculator
{
    ...
    /// </summary>
    /// <param name="parameters">An <see cref="ParameterMapping"/> object containing the front and back distances and the stretch factor used in the calculation</param>
    ...
    protected static float GetVerticalOffset(ParameterMapping parameters) => Math.Clamp((parameters.FrontDistance - parameters.BackDistance) * parameters.Stretch, -1f, 1f);
    ...
    /// </summary>
    /// <param name="parameters">An <see cref="ParameterMapping"/> object containing the right and left distances and the stretch factor used in the calculation.</param>
    ...
    protected static float GetHorizontalOffset(ParameterMapping parameters) => Math.Clamp((parameters.RightDistance - parameters.LeftDistance) * parameters.Stretch, -1f, 1f);
    ...
    /// <param name="parameters">The leash parameters, including stretch, front distance, right distance, left distance, and back distance.</param>
    ...
    protected static float GetHorizontalLook(ParameterMapping parameters, Thresholds thresholds, float horizontalOffset) {
        if (parameters.Stretch <= thresholds.TurningThreshold || parameters.FrontDistance >= thresholds.TurningGoal) {
            return 0f;
        }

        var turn = thresholds.TurningMultiplier * horizontalOffset;
    ...
    /// </summary>
    /// <param name="parameters">The current leash parameters, including the stretch value.</param>
    /// <param name="thresholds">The threshold settings that define the minimum and maximum stretch values for running.</param>
    /// <param name="wasRunningPreviously">Additional leash data, including the current running state.</param>
    /// <returns><see langword="true"/> if the system should transition to a running state; otherwise, <see langword="false"/>.</returns>
    protected static bool ShouldRun(ParameterMapping parameters, Thresholds thresholds, bool wasRunningPreviously) {
        var shouldRun = parameters.Stretch > thresholds.RunningUpperThreshold;
    ...
    /// </summary>
    /// <param name="parameters">The leash parameters, including its grabbed state and stretch value.</param>
    ...
    protected static bool LeashActive(ParameterMapping parameters, Thresholds thresholds) => parameters.IsGrabbed && parameters.Stretch > thresholds.StretchThreshold;
    ...
    /// </summary>
    /// <param name="parameters">The leash parameters, including the distances to the colliders.</param>
    /// <returns><see langword="true" /> if the distances are 0; otherwise, <see langword="false"/>.</returns>
    public static bool IsZeroColliderDistance(ParameterMapping parameters) => parameters is { RightDistance: 0, LeftDistance: 0, FrontDistance: 0, BackDistance: 0 };
...
internal class PositionLeashCalculator : BaseLeashCalculator
{
    ...
    /// <param name="parameters">The threshold settings.</param>
    /// <param name="previous">Previous set of <see cref="BaseLeashCalculator.MovementData"/> that this instance should be compared to.</param>
    /// <returns>An instance of <see cref="BaseLeashCalculator.MovementData"/> that contains the movement data.</returns>
    public static MovementData GetLeashData(IController controller, ParameterMapping parameters, ref MovementData previous) {
        ...
        var verticalOffset = GetVerticalOffset(parameters);
        var horizontalOffset = GetHorizontalOffset(parameters);
        var horizontalLook = GetHorizontalLook(parameters, controller.Thresholds, horizontalOffset);
        var shouldRun = ShouldRun(parameters, controller.Thresholds, previous.ShouldRun);
...
internal class StretchLeashCalculator : BaseLeashCalculator
{
    ...
    public static MovementData GetLeashData(IController controller, ParameterMapping parameters, ref MovementData previous) {
        ...
        var direction = new Vector2(GetHorizontalOffset(parameters), GetVerticalOffset(parameters));
        ...
        var horizontalLook = GetHorizontalLook(parameters, controller.Thresholds, direction.X);
        var shouldRun = ShouldRun(parameters, controller.Thresholds, previous.ShouldRun);
